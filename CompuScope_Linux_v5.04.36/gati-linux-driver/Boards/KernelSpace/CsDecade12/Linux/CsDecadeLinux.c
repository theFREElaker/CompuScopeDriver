      ...
      case CS_IOCTL_START_ACQUISITION:
         {
            ...
            if (0 == iRet) {
               ...
               Abort (FdoData);
               DecadeAcquire (FdoData, &StartAcqParams);
            }
         }
         break;
      ...
      case CS_IOCTL_FLASH_WRITE:
         {
            ...
            if ((uInt32)-1 != ioStruct.u32Sector)
               iRet = DecadeFlashWriteSector (FdoData, ioStruct.u32Sector, ioStruct.u32Offset, kbuf, ioStruct.u32WriteSize);
            else {
      ...
      case CS_IOCTL_FLASH_READ:
         {
            ...
            if (!(kbuf = kmalloc (ioStruct.u32ReadSize, GFP_KERNEL)))
               break;
            if (DecadeFlashRead (FdoData, ioStruct.u32Addr, kbuf, ioStruct.u32ReadSize) != CS_SUCCESS) {
               kfree (kbuf);
               break;
            }
...
void  GetDeviceIdList (uInt16* u16DevIdTable, uInt32 u32TableSize)
{
   ...
   u16DevIdTable[0] = CS_DEVID_DECADE12;
}
