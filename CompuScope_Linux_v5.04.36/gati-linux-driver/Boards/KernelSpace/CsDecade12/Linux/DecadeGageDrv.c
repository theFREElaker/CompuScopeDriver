...
#define ALTMAP_BARINDEX 0x5 // 000101 (6 Pci Bar, 1=memory map)
...
struct file_operations CsFastBlFops = {
   ...
   .read    = GageRead,
   .write   = GageWrite,
   .mmap    = Gage_mmap,  //stm_dma_mmap
};
/**************************************************************************
 *
 **************************************************************************/
int Gage_mmap(struct file *filp, struct vm_area_struct *vmap)
{
   PFDO_DATA FdoData = (PFDO_DATA) filp->private_data;
   int ret = 0;
   //GEN_PRINT("Gage_mmap \n" );   
   
   // User request mapping (shared) CardState buffer ?
   if (FdoData->m_bGetCardState)
   {
      ret = Gage_mmap_CardState(filp, vmap);
      if (ret < 0) {
         //printk(KERN_CRIT "%s: remap of shared memory failed, %d\n", __func__, ret);
         return(ret);
      }
      FdoData->m_bGetCardState = FALSE;
   }
   else
      stm_dma_mmap(filp, vmap);
   return(0);
}
 
int Gage_mmap_CardState(struct file *file, struct vm_area_struct *vma)
{
   int ret = 0;
   PFDO_DATA FdoData = (PFDO_DATA) file->private_data;
   unsigned long length = vma->vm_end - vma->vm_start;
   
   //printk(KERN_INFO "%s(%ld): FdoData %p \n", __func__, vma->vm_end - vma->vm_start, FdoData);
   //printk(KERN_INFO "%s: length %d, vm_start %p, vm_end %p kaddrp %p \n", __func__, length, 
   //             vma->vm_start, vma->vm_end, FdoData->kMapAddr);

    // Map the allocated memory into the calling processes address space.
    if (FdoData->kMapAddr)
    {
       ret = remap_pfn_range(vma, vma->vm_start, virt_to_phys((void *)FdoData->kMapAddr) >> PAGE_SHIFT,
               length, vma->vm_page_prot);
       if (ret < 0) {
           printk(KERN_CRIT "%s: remap of shared memory failed, %d\n", __func__, ret);
           return(ret);
       }
       FdoData->m_pCardState = (void *)FdoData->kMapAddr;
       FdoData->m_pCardState->bPciExpress = FdoData->CardState.bPciExpress;
       FdoData->m_pCardState->u16DeviceId = FdoData->CardState.u16DeviceId;
    }
    else
    {
       printk(KERN_CRIT "%s: shared memory not found!, %d\n", __func__, ret);
       return(-1);
    }
...
int GageAllocCardState(PFDO_DATA  FdoData)
{
   int length = sizeof(CSCARD_STATE);
   int ibufSize;   
   int i = 0;
        
   //GEN_PRINT("GageAllocCardState: CSCARD_STATE %d \n", length);
   /*
      Allocate at least size of cardstate round up to page + extra pages.
      The extra pages allow the structure which may not page aligned to be fit in the mem area.
   */
   length = (length + PAGE_SIZE + 1) & PAGE_MASK;
   ibufSize = length + PAGE_SIZE - 1;
   FdoData->kAddr = kmalloc(ibufSize, GFP_KERNEL);
   if (!FdoData->kAddr) {
      printk(KERN_CRIT "%s: Couldn't allocate kernel memory to share\n", __func__);
      return(-ENOMEM);
   }
    
   /* Address page aligned area is the one we need */
   FdoData->kMapAddr = (void *)(((unsigned long)FdoData->kAddr + PAGE_SIZE - 1) & PAGE_MASK);
   memset(FdoData->kMapAddr, 0, length);
   GEN_PRINT("%s: <%p, %p, %d, %d, %d> \n", __func__, FdoData->kAddr, 
                FdoData->kMapAddr, ibufSize, length, (int)sizeof(CSCARD_STATE));

   for (i = 0; i < FdoData->u32kMapSize; i+= PAGE_SIZE) {
      if (!PageReserved (virt_to_page(FdoData->kMapAddr + i)))
         SetPageDirty (virt_to_page(FdoData->kMapAddr + i));   
   }
   
   FdoData->u32kMapSize = ibufSize;
   
   return(0);
}
int GageDeallocCardState(PFDO_DATA  FdoData)
{
   int i = 0;
   
   GEN_PRINT("GageDeallocCardState  <%p,  %p, %d> \n", 
               FdoData->kAddr, FdoData->kMapAddr, FdoData->u32kMapSize);
   
   if (FdoData->kMapAddr)
   {
      //if ( FdoData->kMapAddr==FdoData->m_pCardState )
         FdoData->m_pCardState = &FdoData->CardState;
      ...
      kfree(FdoData->kAddr);
      FdoData->kAddr = NULL;
      FdoData->kMapAddr = NULL;
      FdoData->u32kMapSize = 0;   
   }
   //GEN_PRINT("%s: done \n", __func__);
   return(0);
...
int GageFindDevice (void)
{
   ...
   while (u16DevId[j] != 0) {
      do {
         ...
         if (pciDev)
         {
            ...
            g_FdoArray[i].m_pCardState  = &g_FdoArray[i].CardState;
   ...
   if (g_CardsCount.u32NumOfCards > 0) {
      ...
      for (i=0; i < g_CardsCount.u32NumOfCards; i++) {
         ...
         ret = GageAllocCardState(FdoData);
         if (ret != 0) {
...
void TestPciCapabilities (PFDO_DATA  FdoData)
{
   ...
   if ((pos = pci_find_capability (dev, PCI_CAP_ID_EXP))) {
      ...
      FdoData->m_pCardState->bPciExpress = TRUE;
...
int GageMapHWResource (PFDO_DATA FdoData)
{
   ...
   for (i = 0; i < 6; i++) {
      if (FdoData->CardState.bPciExpress) {
         ...
         GEN_PRINT ("Non PCIe card!!\n");
         continue;
      }

      FdoData->MemPhysAddress[j].QuadPart = pci_resource_start (pciDev, i);
      FdoData->MemLength[j]               = pci_resource_len (pciDev, i);

      if (request_mem_region (FdoData->MemPhysAddress[j].QuadPart, FdoData->MemLength[j], CS_SYMLINK_NAME) == NULL) {
      ...
      FdoData->MapAddress[j] = ioremap_nocache (FdoData->MemPhysAddress[j].QuadPart, FdoData->MemLength[j]);
      ...
      /* GEN_PRINT ("Virtual base address %d start  = 0x%p\n", j, FdoData->MapAddress[j]); */
      j++;
   }
   if (dma_set_mask (&FdoData->PciDev->dev, DMA_BIT_MASK(64))) {
      GEN_PRINT ("Gage dma_set_mask failed!\n");
      GageUnMapHWResource (FdoData);
      ret = -EPERM;
   }
OnError:
...
int GageUnMapHWResource (PFDO_DATA FdoData)
{
   ...
   if (FdoData->IrqLine) {
      GEN_PRINT ("CsDecadeG12%d Freeing IRQ %2d\n", FdoData->u32CardIndex, FdoData->PciDev->irq);
      free_irq (FdoData->PciDev->irq, (void *) FdoData);
      FdoData->IrqLine = 0;
   }

   for (i = 0; i <NUM_MEMMORYMAP; i++) {
      if (FdoData->MapAddress[i]) {
         iounmap (FdoData->MapAddress[i]);
         FdoData->MapAddress[i] = 0;
      }
      if (FdoData->MemPhysAddress[i].QuadPart) {
         release_mem_region (FdoData->MemPhysAddress[i].QuadPart, FdoData->MemLength[i]);
...
#ifdef USE_SHARED_CARDSTATE
      GageDeallocCardState(g_FdoTable[i]);      
#endif      
   }
   if (g_pGageProcDir) {
      char str[256];
...
#endif
{
   ...
   switch (IoctlCmd) {
      ...
      case CS_IOCTL_WRITE_PCI_REGISTER:
         {
            ...
            _WriteRegister (FdoData, PioStruct->u32Offset, PioStruct->u32RegVal);
         }
         break;
      ...
      case CS_IOCTL_WRITE_NIOS_REGISTER:
         {
            ...
            ioStruct.i32Status = WriteRegisterThruNios (FdoData, ioStruct.u32Data, ioStruct.u32Command, ioStruct.i32Timeout);
      ...
      case CS_IOCTL_WRITE_NIOS_REGISTER_EX:
         {
            ...
            iRet = copy_from_user (&ioStruct, (void __user *)ioctl_arg, sizeof (io_READWRITE_NIOS_REGISTER));
            ioStruct.i32Status = WriteRegisterThruNiosEx (FdoData, ioStruct.u32Data, ioStruct.u32Command, ioStruct.i32Timeout, &ioStruct.u32RetVal);
            iRet = copy_to_user ((void __user *) ioctl_arg, &ioStruct, sizeof (io_READWRITE_NIOS_REGISTER));
         }
         break;
      ...
      case CS_IOCTL_GET_CARDSTATE_PTR:
         {
            FdoData->m_bGetCardState = true;                //mmap will follow
         }
         break;
      default:
...
/**************************************************************************
 * Read data direct from firmware
 **************************************************************************/
#if 0
ssize_t GageRead (struct file *filp, char __user *pBuffer, size_t Size, loff_t *Offset)
{
   ...
   uInt32   i = 0;
   uInt32   u32NbFifoRead = Size / 4;
   uInt32   *p32Temp = (uInt32 *) pBuffer;

   uInt32 u32Index = 0;
   for (i = 0; i < u32NbFifoRead; i++)
   {
      // Fill 4 samples in buffer with one READ FIFO
      *p32Temp = ReadGageRegister(FdoData, 0xD0 + u32Index);
      u32Index += 4;
      u32Index %= 32;
      p32Temp++;
   }
...
ssize_t GageRead (struct file *filp, char __user *pBuffer, size_t Size, loff_t *Offset)
{
   ...
   */
   ...
   if (FdoData->bDataTransferIntrEn) {
      iRet = wait_event_interruptible_timeout (FdoData->DmaWaitQueue, ! FdoData->bDmaIntrWait,  msecs_to_jiffies (2000));
      if (0 == iRet) {
 ...
 **************************************************************************/
#if (LINUX_VERSION_CODE < KERNEL_VERSION (4,6,0))
ssize_t GageWrite (struct file *filp, char __user *pBuffer, size_t Size, loff_t *Offset)
#else
ssize_t GageWrite (struct file *filp, const char __user *pBuffer, size_t Size, loff_t *Offset)
#endif
{
   return 0;
}
