...
#define DECADEFLASH_SECTOR_SIZE   0x80000   // The Decade Flash sector in bytes
...
int32   FlashProgramBuffer (PFDO_DATA FdoData, uInt32 u32WordAddr, void *pBuffer, uInt32 u32BufferSize)
{
   ...
   uInt32   u32Val = 0;
   uInt32   u32StartSectorAddr = (4*u32WordAddr/DECADEFLASH_SECTOR_SIZE)*DECADEFLASH_SECTOR_SIZE;
   uInt32   u32EndSectorAddr = ((4*u32WordAddr + u32BufferSize - 1)/DECADEFLASH_SECTOR_SIZE)*DECADEFLASH_SECTOR_SIZE;
...
int32 DecadeFlashRead (PFDO_DATA FdoData,  uInt32 u32Addr, void *pBuffer,  uInt32 u32BufferSize)
{
   ...
   //   if (u32BufferSize > DECADEFLASH_SECTOR_SIZE)
   //   return CS_FLASH_SECTORCROSS_ERROR;
...
int32 DecadeFlashWriteSector (PFDO_DATA FdoData,  uInt32 u32Addr, uInt32 u32Offset, void *pBuffer,  uInt32 u32BufferSize)
{
   ...
   if ((u32BufferSize + u32Offset) > DECADEFLASH_SECTOR_SIZE)
   {
      return CS_FLASH_SECTORCROSS_ERROR;
   }
   if (0 != (u32BufferSize % 4))
   ...
   // Erase the sector
   u32SectorAddr = (u32Addr/DECADEFLASH_SECTOR_SIZE) * DECADEFLASH_SECTOR_SIZE;
   u32SectorAddr >>= 2;
