...
#define  HEXAGON_MAX_DMA_DESC_BLOCKS   1024
#define  HEXAGON_MAX_DMA_DESC_MAX_SIZE (0x80000-0x1000)
...
uInt32 AltBuildDmaDescContiguous (PFDO_DATA FdoData, uInt64 PhysAddr, uInt32 u32Size)
{
   ...
   uInt32 current_size = HEXAGON_MAX_DMA_DESC_MAX_SIZE;
   ...
   if (u32Size > HEXAGON_MAX_DMA_DESC_MAX_SIZE) {
      desc_count = u32Size/HEXAGON_MAX_DMA_DESC_MAX_SIZE;
      if ((u32Size % HEXAGON_MAX_DMA_DESC_MAX_SIZE) > 0)
         desc_count += 1;
   }

   // Contiguous memory too big !!!!
   if (desc_count > HEXAGON_MAX_DMA_DESC_BLOCKS)
      desc_count = HEXAGON_MAX_DMA_DESC_BLOCKS;

   if (size_remain < HEXAGON_MAX_DMA_DESC_MAX_SIZE)
   ...
   while (desc_count != 0) {
      ...
      addr.QuadPart += HEXAGON_MAX_DMA_DESC_MAX_SIZE;
      desc_count--;
   }
                                    ...
                                    uInt32 NumberOfElements)
{
   ...
   UPHYSICAL_ADDRESS addr;
	...
	if ( u32DescCount > HEXAGON_MAX_DMA_DESC_BLOCKS )
		 u32DescCount = HEXAGON_MAX_DMA_DESC_BLOCKS;
...
BOOLEAN AltWriteBlockDescriptor (PFDO_DATA FdoData, uInt64 Address, uInt32 Length, BOOLEAN bLastChunk)
{
	...
	uInt32				u32ChunkSize	= HEXAGON_MAX_DMA_DESC_MAX_SIZE;
	...
	if ( Length > HEXAGON_MAX_DMA_DESC_MAX_SIZE )
	{
      ...
      /* GEN_DEBUG_PRINT (("Break Descriptor ------  (size = 0x%x)\n", Length)); */
		...
		while ( u32SizeRemain > 0 && FdoData->XferRead.u32CurrentBlocksCount < (HEXAGON_MAX_DMA_DESC_BLOCKS-1) )
		{
			_WriteRegister( FdoData, ALTERA_REG_DMAWRHA, ChunkAddr.HighPart );
			_WriteRegister( FdoData, ALTERA_REG_DMAWRA, ChunkAddr.LowPart );	//u32StartAddr + j*HEXAGON_MAX_DMA_DESC_MAX_SIZE );
		...
		// The last chunk
		if ( bLastChunk || FdoData->XferRead.u32CurrentBlocksCount == (HEXAGON_MAX_DMA_DESC_BLOCKS-1) )
		{
