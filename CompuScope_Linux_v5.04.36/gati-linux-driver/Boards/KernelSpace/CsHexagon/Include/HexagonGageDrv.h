#ifndef hexagongagedrv_h
#define hexagongagedrv_h
...
#define   NUMBER_MSI_SUPPORTED   32      // Number of MSI message supported
...
typedef struct _PCI_BAR_INFO
{
    ...
    UPHYSICAL_ADDRESS Physical;   // ! BAR Physical Address
...
typedef struct _KSTMDMABUFFER
{
   ...
   UPHYSICAL_ADDRESS  PhysAddr;      // Logical adddress for DMA
...
typedef struct _FDO_DATA
{
   ...
   BUSTYPE          BusType;
   uInt32           u32CardIndex;
   BOARD_VOLATILE_STATE	VolatileState;
   PCIeLINK_INFO    PciExLinkInfo;
   UPHYSICAL_ADDRESS MemPhysAddress[PCI_NUM_BARS];
   ...
   WDFINTERRUPT                    WdfInterrupt[NUMBER_MSI_SUPPORTED]; // Interrupt objects
   PKEVENT                         pkEventAlarm;
   HANDLE                          hCaptureProcess;           // The process that sent StartAcquisition request
   BOOLEAN                         bMsiEnabled;              // MSI interrupt enabled
   uInt16                          u16NumberOfMsiRequested;    // Number of MSI message requested from the inf file
   WDFDMAENABLER                   WdfDmaEnabler;
   FAST_MUTEX                      fMutex;
   UPHYSICAL_ADDRESS               PhysAddrDmaDesc;
   ...
   // Debug DMA speed
   BOOLEAN         bDebug_DMA_Speed;
   uInt32         u32DmaPerfCount;
   ...
   int64         TotalDmaTransferedLength;
   ...
   CSCARD_STATE      *m_pCardState;

   void              *kAddr;              // kmalloc addr
   char              *kMapAddr;           // Page align address
   uInt32            u32kMapSize;
	BOOLEAN				m_bGetCardState;		// map cardstate via mmap
...
} HEXAGON_INTERRUPT_CONTEXT, *PHEXAGON_INTERRUPT_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME (FDO_DATA, GageFdoGetData)
WDF_DECLARE_CONTEXT_TYPE_WITH_NAME (HEXAGON_INTERRUPT_CONTEXT, GetInterruptContext)

DRIVER_INITIALIZE DriverEntry;

EVT_WDF_DRIVER_DEVICE_ADD GageEvtDeviceAdd;

EVT_WDF_DEVICE_FILTER_RESOURCE_REQUIREMENTS   GageEvtFilterAddResourceRequirements;
...
EVT_WDF_INTERRUPT_ISR                GageEvtMessageInterruptIsr;
EVT_WDF_INTERRUPT_DPC                GageEvtInterruptDpc;
EVT_WDF_INTERRUPT_DPC                GageEvtMessageInterruptDpc;
...
#define READ_PCI_CONFIG_SPACE(DeviceObject, Buffer, Offset, Length) ReadWriteConfigSpace(DeviceObject, 0, Buffer, Offset, Length)

#define WRITE_PCI_CONFIG_SPACE(DeviceObject, Buffer, Offset, Length) ReadWriteConfigSpace (DeviceObject, 1, Buffer, Offset, Length)

#ifdef USE_POWER_POLICY
NTSTATUS ToasterWmiRegistration (__in WDFDEVICE Device);

NTSTATUS ToasterFireArrivalEvent (__in WDFDEVICE  Device);
#endif
...
NTSTATUS GageConnectInterrupts               (IN OUT PFDO_DATA FdoData, WDFCMRESLIST   ResourcesRaw, WDFCMRESLIST   ResourcesTranslated);
NTSTATUS GageAllocateSoftwareResources       (IN OUT PFDO_DATA FdoData);
void     GageFreeSoftwareResources           (IN OUT PFDO_DATA FdoData);
BOOLEAN  GageAllocateMemmoryForDmaDescriptor (IN PFDO_DATA   FdoData);
void     GageFreeMemmoryForDmaDescriptor     (IN PFDO_DATA   FdoData);
...
ssize_t GageWrite        (struct file *filp, char __user *buffer, size_t Size, loff_t *Offset);
#else
ssize_t GageWrite        (struct file *filp, const char __user *buffer, size_t Size, loff_t *Offset);
#endif
ssize_t GageReadMaster   (struct file *filp, char __user *buffer, size_t Size, loff_t *Offset);
ssize_t GageReadSlave    (struct file *filp, char __user *buffer, size_t Size, loff_t *Offset);
...
void    EnablePciInterrupts  (PFDO_DATA FdoData);
void    DisablePciInterrupts (PFDO_DATA FdoData);
