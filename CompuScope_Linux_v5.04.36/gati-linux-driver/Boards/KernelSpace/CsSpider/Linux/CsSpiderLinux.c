      ...
      case CS_IOCTL_PRE_DATATRANSFER:
         {
            ...
            pOutTransferEx->i32ActualStartOffset = u32LocalAlignOffset + (FdoData->m_MemEtb >> 1);
      ...
      case CS_IOCTL_START_ACQUISITION:
         {
            ...
            if ( 0 == iRet )
            {
               ...
               Abort( FdoData );
               SpiderAcquire( FdoData, &StartAcqParams );
            }
         }
         break;

         //// RG - added Sept 24, 2009
...
void  GetDeviceIdList( uInt16* u16DevIdTable, uInt32 u32TableSize )
{
   ...
   u16DevIdTable[0] = CS_DEVID_SPIDER;
   u16DevIdTable[1] = CS_DEVID_SPIDER_v12;
   u16DevIdTable[2] = CS_DEVID_SPIDER_PCIE;
   u16DevIdTable[3] = CS_DEVID_ZAP_PCIE;
}
