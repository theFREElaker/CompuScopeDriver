...
#endif
{
   ...
   switch (IoctlCmd)
   {
      ...
      case CS_IOCTL_START_ACQUISITION:
         {
            ...
            if ( 0 == iRet )
            {
               ...
               Abort( FdoData );
               SplendaAcquire( FdoData, &StartAcqParams );
            }
         }
         break;
      ...
      case  CS_IOCTL_PROGRAM_ADDON_FPGA:
         {
            ADDON_FPGA_DESC   ioStruct;
            uInt8* kbuf;

            iRet = copy_from_user ( &ioStruct, (void __user *)ioctl_arg, sizeof (ADDON_FPGA_DESC));
            if (!(kbuf = kmalloc(ioStruct.u32BufferSize, GFP_KERNEL)))
            {
               iRet = -EFAULT;
               break;
            }

            iRet = copy_from_user ( kbuf, (void __user *)ioStruct.pFpgaBuffer, ioStruct.u32BufferSize);

            ioStruct.i32Status = ProgramAddonFpga(FdoData, kbuf, ioStruct.u32BufferSize );
            
            iRet = copy_to_user ((void __user *)ioStruct.pFpgaBuffer, kbuf, ioStruct.u32BufferSize);
            kfree(kbuf);

            iRet = copy_to_user ((void __user *)ioctl_arg, &ioStruct, sizeof (ADDON_FPGA_DESC));
         }
         break;
...
void   GetDeviceIdList( uInt16* u16DevIdTable, uInt32 u32TableSize )
{
   ...
   u16DevIdTable[0] = CS_DEVID_SPLENDA;
   u16DevIdTable[1] = CS_DEVID_SPLENDA_PCIE;
   u16DevIdTable[2] = CS_DEVID_OSCAR_PCIE;
}
...
#ifdef TRACE_DEBUG_ISR
         GEN_DEBUG_PRINT("  IntFifo = 0x%x,", IntFifo);
#endif
