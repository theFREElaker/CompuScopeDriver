...
int ADQ114::Open(HWIF* device)
{
	int success;
	int* rev;
	
    parameter_pll_basefreq = ADQ114_DEFAULT_PLL_FREQUENCY;
    parameter_pll_vcoset = ADQ114_DEFAULT_PLL_VCOSET; 
    ...
    SetTrigLevelResetValue(ADQ114_DEFAULT_LEVEL_TRIGGER_RESET);
	...
	//success = success && SetPll(DEF_FREQ_N_DIV_114, DEF_FREQ_R_DIV, DEF_FREQ_VCO_DIV, DEF_FREQ_CH_DIV);
	success = success && SetTriggerMode(DEF_TRIG_MODE);
	success = success && SetPreTrigWords(0);
	success = success && SetTriggerHoldOffSamples(0);
	success = success && SetWordsPerPage(DEF_WORDS_PER_PAGE_114); //Default for ADQ214 0902_B
    ...
    success = success && WriteRegister(TRIGGER_DELAY_ADDR, 0x0, ADQ114_DEFAULT_TRIGGER_DELAY);
...
int ADQ114::GetADQType(void)
{
    return (int)114;
}

unsigned int ADQ114::SetSampleSkip(unsigned int SkipFactor)
{
...
unsigned int ADQ114::GetSampleSkip(void)
{
    unsigned int value;
...
void* ADQ114::GetPtrData(unsigned int channel)
{
    void* rtnptr = NULL;
...
int ADQ114::ParseSampleData(unsigned int wordNbOffset, unsigned int sampleOffset, unsigned int destOffset, unsigned int maxSamples)
{
	...
	unsigned int bitOffset = wordNbOffset*m_WordNbBits +        //bit address to first sample (considering only one channel's data)
							 (sampleOffset/2)*sampleOffsetStore; // /2 for ADQ114
	...
	unsigned int parsedSamples = wordNbOffset * m_SamplesPerWordNb * 2 + sampleOffset; //Count how many samples have been consumed in page // *2 for ADQ114
	bitOffset = bitOffset % 8;                             //bit address relative first byte
	sampleOffset = sampleOffset % 2;                       //for ADQ114 Even/odd starting point
...
int ADQ114::SetAlgoStatus(int status)
{
	status = (status)? 1 : 0;
	return WriteAlgoRegister(ALGO_ALGO_ENABLED_ADDR, 0, status);
}

int ADQ114::SetAlgoNyquistBand(unsigned int band)
{
	int status = 0;
	if (band <= 1)
...
int ADQ114::SetDataFormat(unsigned int format)
{
	int success;
	...
	case 0: //Packed N-bit data
		success = success && SetNofBits(N_WORDBITS_ADQ114); //Default for ADQ214 R0910
		success = success && SetSampleWidth(N_BITS_ADQ114); //Default for ADQ214 R0910
		break;
	...
	if (success && !SetWordsPerPage(m_WordsNbPerPage))
				success = SetWordsPerPage(DEF_WORDS_PER_PAGE_114);
	return success;
}