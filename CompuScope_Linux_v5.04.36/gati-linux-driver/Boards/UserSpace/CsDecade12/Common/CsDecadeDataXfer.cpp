...
int32	CsDecade::ProcessDataTransfer(in_DATA_TRANSFER *InDataXfer)
{
	...
	if ( m_bFFT_enabled || (TxMODE_DATA_32 & pInParams->u32Mode) )
	...
	if (m_iOCT_ModeSelect)
		InXferEx.u32FifoMode = MODESEL0;  // Force to use passthough (single channel) in OCT mode
...
int32	CsDecade::ProcessDataTransferDebug(in_DATA_TRANSFER *InDataXfer)
{
	int32						i32Status = CS_SUCCESS;
	PIN_PARAMS_TRANSFERDATA		pInParams;
	POUT_PARAMS_TRANSFERDATA	pOutParams;
...
////////////////////////////////////////////////////////////////////////
//
int32	CsDecade::ProcessDataTransferModeAveraging(in_DATA_TRANSFER *InDataXfer)
{
	PIN_PARAMS_TRANSFERDATA		pInParams;
	POUT_PARAMS_TRANSFERDATA	pOutParams;
	int64						i64PreDepth;
...
int32	CsDecade::ProcessSoftwareAveraging( int64 i64StartAddress, uInt16 u16Channel, int32 *pi32ResultBuffer, uInt32 u32SamplesInBuffer )
{
	int32	i32Status = CS_SUCCESS;
...
//
//-----------------------------------------------------------------------------
int32 CsDecade::CreateThreadAsynTransfer()
{
...
void CsDecade::AsynDataTransferDone()
{
	CsDecade *pDevice = GetCardPointerFromChannelIndex( m_pSystem->u16ActiveChannelTx );
...
//
//-----------------------------------------------------------------------------
int32	CsDecade::CsTransferTimeStamp(in_DATA_TRANSFER *InDataXfer)
{
	PIN_PARAMS_TRANSFERDATA		pInParams;
	POUT_PARAMS_TRANSFERDATA	pOutParams;
