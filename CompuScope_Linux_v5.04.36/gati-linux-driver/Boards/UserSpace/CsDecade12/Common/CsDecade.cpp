...
// CsDecase.cpp : Defines the entry point for the DLL application.
//
...
CSDECADE_BOARDCAPS		g_BoardCaps;
#endif
...
PCSDECADE_BOARDCAPS		CsDllGetSystemCapsInfo( DRVHANDLE DrvHdl );
...
int32 CsDecade::DumpBuffer(char *Label, uInt8 *pBuf, int32 size)
{
   int i = 0;
   if (pBuf && size)
   {
...
//
//-----------------------------------------------------------------------------
typedef SharedMem< CSDECADE_BOARDCAPS > SHAREDxyG8CAPS;
int32 CsDrvAcquisitionSystemInit( DRVHANDLE	DrvHdl, BOOL bSystemDefaultSetting )
{
	...
	for (uInt32 i = 0; i < Globals.it()->g_StaticLookupTable.u16SystemCount; i++)
	{
		if ( pDrvSystem[i].DrvHandle == DrvHdl )
		{
			...
			qsort(pCaps->it()->RangeTable, DECADE_MAX_RANGES, sizeof(CSRANGETABLE),CompareInputRange );
			pDrvSystem[i].pCapsInfo = pCaps;
#else
			g_pBaseHwDll->CsDrvGetParams( DrvHdl, CS_DRV_BOARD_CAPS, &g_BoardCaps );
			qsort(&g_BoardCaps.RangeTable, DECADE_MAX_RANGES, sizeof(CSRANGETABLE),CompareInputRange );
#endif
		}
	}
	return i32Status;
...
#define DECADE_MIN_SEG_SIZE					16		// Should be multiple of 8
...

{
		  ...
		  ( (pAcqCfg->u32Mode & (CS_MODE_DUAL|CS_MODE_SINGLE_CHANNEL2)) == (CS_MODE_DUAL|CS_MODE_SINGLE_CHANNEL2) ) )
	{
		if ( u32Coerce )
		{
			pAcqCfg->u32Mode = ( u32AcqModeValid & CS_MODE_DUAL ) ? CS_MODE_DUAL : CS_MODE_SINGLE;
			i32Status = CS_CONFIG_CHANGED ;
		}
		else
			return CS_INVALID_ACQ_MODE;
	...
	if (0 != pAcqCfg->u32ExtClk && 1000 != pAcqCfg->u32ExtClkSampleSkip && 500 != pAcqCfg->u32ExtClkSampleSkip)
	{
		if (u32Coerce)
		{
			pAcqCfg->u32ExtClkSampleSkip = 1000;
			i32Status = CS_CONFIG_CHANGED;
		}
		else
			return CS_INVALID_EXTCLK_SAMPLESKIP;
	}
	...
	int64	i64MaxHwDepthCounter = DECADE_MAX_HW_DEPTH_COUNTER;
	i64MaxHwDepthCounter *= 32;
	i64MaxHwDepthCounter /= (pAcqCfg->u32Mode&CS_MASKED_MODE);
								  ...
								  uInt32 Coerce )
{
	...
	for ( uInt32 k = 0; k < pSysCfg->pAChannels->u32ChannelCount; k ++, pChanCfg++ )
	{
		...
		//----- Term
		int		j;
		PCSDECADE_BOARDCAPS pCaps = CsDllGetSystemCapsInfo(DrvHdl);
		for( j = 0; j < DECADE_COUPLING; j++ )
		{
			if ( pChanCfg->u32Term == pCaps->CouplingTable[j].u32Coupling )
				break;
		}
		if ( DECADE_COUPLING == j )
		{
			if ( Coerce )
			{
								   ...
								   uInt32 *BufferSize )
{
	...
	if ( pSystemConfig == NULL )//----- return the complete table
	{
		...
		for (i = 0; i < DECADE_MAX_SR_COUNT; i ++ )
		{
								   ...
								   uInt32 *BufferSize )
{
	...
	if ( u16Input > 0 )
	{
		u32MaxRange =  DECADE_MAX_RANGES;
		pLocRangeTable = pCaps->RangeTable;
	}
	else
	{
		u32MaxRange =  DECADE_EXT_TRIG_RANGES;
	...
	if ( pSystemConfig == NULL )
	{
		...
		for (i = 0; i < DECADE_EXT_TRIG_RANGES; i ++ )
		{
	...
	else
	{
		...
		else
		{
			...
			if (NULL != pRangesTbl)
			{
				...
				{
					for (i = 0, j = 0; i < DECADE_MAX_RANGES && j < u32NumOfElement; i++)
					{
						if ((pCaps->RangeTable[i].u32Reserved  & ImpedanceMask) == ImpedanceMask)
						{
								   ...
								   uInt32 *BufferSize )
{
	...
	else						//----- 2nd call return the table
	{
		...
		else
		{
			*u32Terminations = 0;
			PCSDECADE_BOARDCAPS pCaps = CsDllGetSystemCapsInfo(DrvHdl);
			for( int j = 0; j < DECADE_COUPLING; j++ )
			{
...
int32	CsDrvGetTriggerResolution( DRVHANDLE , uInt32 *pBuffer,  uInt32 *BufferSize )
{
	...
	if ( NULL != pBuffer )
	{
		if ( IsBadWritePtr( pBuffer, sizeof( uInt32) ) )
			...
			*pBuffer = DECADE_DEPTH_INCREMENT;
	}
...
int32	CsDrvGetTriggerDelayIncrement( DRVHANDLE, uInt32 *pBuffer,  uInt32 *BufferSize )
{
	...
	{
		*BufferSize = sizeof(uInt32);
		*pBuffer = (uInt32)(DECADE_DELAY_INCREMENT * 2);		// Single channel
	}
...
int64 ValidateExtClockRate ( DRVHANDLE DrvHdl, PCSSYSTEMINFO , PCSSYSTEMCONFIG pSysCfg )
{
	...
	if ( pAcqCfg->u32ExtClk )
	{
		...
		for ( uInt32 i = 0; i < DECADE_MODE_COUNT; i ++ )
		{
			if ( pAcqCfg->u32Mode & pCaps->ExtClkTable[i].u32Mode )
			{
...
int32	_CsDrvGetHwOptionText( uInt32 u32BoardType, PCS_HWOPTIONS_CONVERT2TEXT pHwOptionText )
{
	...
	{
		char szBaseboardOptions[][HWOPTIONS_TEXT_LENGTH] =
		{
			"1V07 Addon",				 //bit  0
		};

		pText = (char *) szBaseboardOptions;
		pHwOptionText->u32RevMask = DECADE_BBHW_OPT_FWREQUIRED_MASK;
		nSize = sizeof(szBaseboardOptions) / HWOPTIONS_TEXT_LENGTH;
	}
	else
	{
	...
	for (uInt32 i = 0; i < nSize; i++)
	{
		...
		{
			strcpy_s(pHwOptionText->szOptionText, sizeof(pHwOptionText->szOptionText), &pText[i*HWOPTIONS_TEXT_LENGTH]);
			return CS_SUCCESS;
		}
	}
...
int32	_CsDrvGetFwOptionText( PCS_FWOPTIONS_CONVERT2TEXT pFwOptionText )
{
	...
	{
		if ( (1<<pFwOptionText->u8BitPosition) & DECADE_VALID_EXPERT_FW )
		{
...
int32	_CsDrvGetBoardType( uInt32 *u32MinBoardType, uInt32 *u32MaxBoardType )
{
	...
	*u32MinBoardType = CSDECADE_FIRST_BOARD;
	*u32MaxBoardType = CSDECADE_LAST_BOARD;
	...
	case CS_FLASHINFO:
		{
			...
			{
				// There is no EEPROM on the Decade addon.
				...
				pCsFlashLayoutInfo->u32FlashFooterOffset	= FIELD_OFFSET( DECADE_FLASH_LAYOUT, BoardData) + FIELD_OFFSET( DECADE_MISC_DATA, AddonInfo );
			}
			else
			{
				pCsFlashLayoutInfo->u32FlashType			= 1;
				pCsFlashLayoutInfo->u32NumOfImages			= DECADE_NUMBER_OF_IMAGES-1;	// -1 to remove the SafeBoot image.
				pCsFlashLayoutInfo->u32ImageStructSize		= sizeof(DECADE_FLS_IMAGE_STRUCT);
				pCsFlashLayoutInfo->u32WritableStartOffset	= FIELD_OFFSET( DECADE_FLASH_LAYOUT, FlashImage0 );
				pCsFlashLayoutInfo->u32CalibInfoOffset		= FIELD_OFFSET( DECADE_FLS_IMAGE_STRUCT, Calib );
				pCsFlashLayoutInfo->u32FwInfoOffset			= FIELD_OFFSET( DECADE_FLS_IMAGE_STRUCT, FwInfo );
				pCsFlashLayoutInfo->u32FlashFooterOffset	= FIELD_OFFSET( DECADE_FLASH_LAYOUT, BoardData) + FIELD_OFFSET( DECADE_MISC_DATA, BaseboardInfo );
				pCsFlashLayoutInfo->u32FwBinarySize			= DECADE_FWIMAGESIZE;
			}
		}
		return CS_SUCCESS;
...
BOOL _CsDrvIsMyBoardType( uInt32 u32BoardType )
{
	...
	if( u32BoardType >= CSDECADE_FIRST_BOARD && u32BoardType <= CSDECADE_LAST_BOARD )
		return TRUE;
	else
		return FALSE;
...
{
	CsDecade	*pDev = (CsDecade *) g_pBaseHwDll->GetAcqSystemCardPointer( DrvHdl );
