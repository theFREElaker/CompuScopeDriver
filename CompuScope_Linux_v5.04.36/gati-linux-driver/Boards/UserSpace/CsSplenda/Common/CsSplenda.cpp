...
CSSPLENDABOARDCAPS	g_BoardCaps;
#endif
...
PCSSPLENDABOARDCAPS		CsDllGetSystemCapsInfo( DRVHANDLE DrvHdl );
...
//
//-----------------------------------------------------------------------------
typedef SharedMem< CSSPLENDABOARDCAPS > SHAREDSplendaCAPS;
int32 CsDrvAcquisitionSystemInit( DRVHANDLE	DrvHdl, BOOL bSystemDefaultSetting )
{
	...
	for (uInt32 i = 0; i< Globals.it()->g_StaticLookupTable.u16SystemCount; i ++ )
	{
		...
		{
			// Save a copy of current AcqusisitonConfig
			...
			SHAREDSplendaCAPS* pCaps = new SHAREDSplendaCAPS( strSharedCapsName, true );
			g_pBaseHwDll->CsDrvGetParams( DrvHdl, CS_DRV_BOARD_CAPS, pCaps->it() );
			qsort(pCaps->it()->RangeTable, CSPLNDA_MAX_RANGES, sizeof(CSRANGETABLE),CompareInputRange );
			pDrvSystem[i].pCapsInfo = pCaps;
#else
			g_pBaseHwDll->CsDrvGetParams( DrvHdl, CS_DRV_BOARD_CAPS, &g_BoardCaps );
			qsort(&g_BoardCaps.RangeTable, CSPLNDA_MAX_RANGES, sizeof(CSRANGETABLE),CompareInputRange );
#endif
		}
	}
	return i32Status;
...
int32 CsDrvAcquisitionSystemCleanup(DRVHANDLE DrvHdl)
{
	...
	for (uInt32 i = 0; i< Globals.it()->g_StaticLookupTable.u16SystemCount; i ++ )
	{
		...
		{
			delete ((SHAREDSplendaCAPS* ) pDrvSystem[i].pCapsInfo);
		}
	}
#endif
...
{	
	...
	case CAPS_BOOTIMAGE0:
		if ( CS_DEVID_SPLENDA_PCIE != pSysInfoEx->u16DeviceId && CS_DEVID_OSCAR_PCIE != pSysInfoEx->u16DeviceId )
...
int32 CsDrvSetParams(DRVHANDLE DrvHdl, uInt32 ParamID, void *ParamBuffer)
{
	...
	switch (ParamID)
	{
		...
		case CS_DEFAULTUSE_DACCALIBTABLE:
		case CS_MULREC_AVG_COUNT:
			if ( IsBadReadPtr( ParamBuffer, sizeof(uInt32) ) )
				return CS_INVALID_POINTER_BUFFER;
			break;
...
PCSSPLENDABOARDCAPS CsDllGetSystemCapsInfo( DRVHANDLE DrvHdl)
{
	...
	for (uInt32 i = 0; i < Globals.it()->g_StaticLookupTable.u16SystemCount; i++)
	{
		...
		{
#ifdef USE_SHARE_BOARDCAPS
			return ((SHAREDSplendaCAPS*)(pDrvSystem[i].pCapsInfo))->it();
#else
			return &g_BoardCaps;
#endif
		}
...
#define CSPLNDA_MIN_SEG_SIZE					8		// Should be multiple of 8
//-----------------------------------------------------------------------------
//
//-----------------------------------------------------------------------------
								  ...
								  uInt32 Coerce )
{
	...
	for ( uInt32 k = 0; k < pSysCfg->pAChannels->u32ChannelCount; k ++, pChanCfg++ )
	{
		...
		//----- Term
		int		j;
		PCSSPLENDABOARDCAPS pCaps = CsDllGetSystemCapsInfo(DrvHdl);
		for( j = 0; j < CSPLNDA_COUPLING; j++ )
		{
			if ( pChanCfg->u32Term == pCaps->CouplingTable[j].u32Coupling )
				break;
		}
		if ( CSPLNDA_COUPLING == j )
		{
			if ( Coerce )
			{
		...
		for ( i = 0; i < CSPLNDA_MAX_RANGES; i ++ )
		{
							...
							POUT_PARAMS_TRANSFERDATA	OutParams )
{
	...
	const uInt32	cu32ValidTxMode	= ( TxMODE_SLAVE | TxMODE_DATA_16 | TxMODE_DATA_ONLYDIGITAL | 
										TxMODE_DATA_32 | TxMODE_DATA_FFT);
	...
	if ( InParams.u32Mode == 0 || InParams.u32Mode == TxMODE_DEBUG_DISP || ((InParams.u32Mode & cu32ValidTxMode ) != 0 ) )
	{
		//
								   ...
								   uInt32 *BufferSize )
{
	...
	if ( pSystemConfig == NULL )//----- return the complete table
	{
		...
		for (i = 0; i < CSPLNDA_MAX_SR_COUNT; i ++ )
		{
								   ...
								   uInt32 *BufferSize )
{
	...
	else
	{
		...
		if ( Input == 0 )
		{
			for (i = 0; i < CSPLNDA_EXT_TRIG_RANGES; i ++ )
			{
		...
		else
		{
			...
			else
			{
				...
				for (i = 0, j= 0; i < CSPLNDA_MAX_RANGES && j < u32NumOfElement; i++ )
				{
					if ( (pCaps->RangeTable[i].u32Reserved  & ImpedanceMask) == ImpedanceMask )
					{
								   ...
								   uInt32 *BufferSize )
{
	...
	{
		for( uInt32 i = 0; i< CSPLNDA_IMPED; i++ )
		{
								   ...
								   uInt32 *BufferSize )
{
	...
	for( uInt32 i = 0; i< CSPLNDA_COUPLING; i++ )
	{
								   ...
								   uInt32 *BufferSize )
{
	...
	else						//----- 2nd call return the table
	{
		...
		else 
		{
			*u32Terminations = 0;
			PCSSPLENDABOARDCAPS pCaps = CsDllGetSystemCapsInfo(DrvHdl);
			for( int j = 0; j < CSPLNDA_COUPLING; j++ )
			{
...
int32	CsDrvGetBoardTriggerEngines( DRVHANDLE DrvHdl, uInt32 CapsId, uInt32 *pBuffer,  uInt32 *BufferSize )
{
	...
	uInt32 u32ChanPerBoard = pSysInfo->u32ChannelCount / pSysInfo->u32BoardCount;
	*pBuffer = 1 + u32ChanPerBoard * (*pBuffer);
	*BufferSize = sizeof(uInt32);
...
int32	CsDrvGetMaxSegmentPadding( DRVHANDLE , uInt32 *pBuffer,  uInt32 *BufferSize )
{
	...
	if ( NULL != pBuffer )
	{
		*pBuffer = 	CSPLNDA_PRE_TRIG_PIPELINE + 
			        MAX_SPLNDA_TAIL_INVALID_SAMPLES + 
					MAX_SPLNDA_TRIGGER_ADDRESS_OFFSET + 
					(BYTESRESERVED_HWMULREC_TAIL/sizeof(uInt16)); // Max Segment Padding
	}
...
//
//-----------------------------------------------------------------------------
int32	CsDrvGetTriggerResolution( DRVHANDLE DrvHdl, uInt32 *pBuffer,  uInt32 *BufferSize )
{
	if (NULL == BufferSize)
		return (CS_INVALID_PARAMETER);
	if ( IsBadWritePtr( BufferSize, sizeof( uInt32) ) )
	...
	if ( NULL != pBuffer )
	{
		...
		if ( CS_DEVID_SPLENDA_PCIE == pSysInfoEx->u16DeviceId || CS_DEVID_OSCAR_PCIE == pSysInfoEx->u16DeviceId )
			*pBuffer = CSPLNDA_PCIE_TRIGGER_RES;
		else
			*pBuffer = CSPLNDA_TRIGGER_RES;
	}
...
int64 ValidateSampleRate ( DRVHANDLE DrvHdl, PCSSYSTEMINFO , PCSSYSTEMCONFIG pSysCfg )
{
	...
	if ( pAcqCfg->u32ExtClk )
	{
		...
		for ( uInt32 i = 0; i < CSPLNDA_MODE_COUNT; i ++ )
		{
			if ( pAcqCfg->u32Mode & pCaps->ExtClkTable[i].u32Mode )
			{
	...
	else
	{
		i64CoerceSampleRate = g_pBaseHwDll->CoerceSampleRate( pAcqCfg->i64SampleRate, 
			                                                  ...
			                                                  0, 
															  pCaps->SrTable, 
															  CSPLNDA_MAX_SR_COUNT * sizeof( CSSAMPLERATETABLE_EX ) );
	}
...
int32	_CsDrvGetHwOptionText( uInt32 u32BoardType, PCS_HWOPTIONS_CONVERT2TEXT pHwOptionText )
{
	...
	if ( pHwOptionText->bBaseBoard )
	{
		...
		pHwOptionText->u32RevMask = CSPLNDA_BBHW_OPT_FWREQUIRED_MASK;

		// For Oscar
		char szBaseBoardptions[][HWOPTIONS_TEXT_LENGTH] =  {	"Oscar BaseBoard" };      //bit  0

		if ( 0 == pHwOptionText->u8BitPosition )
		{
			strcpy_s(pHwOptionText->szOptionText, sizeof(pHwOptionText->szOptionText), szBaseBoardptions[0] );
		}
		else
		{
	...
	// For Oscar			100 MSmpl 	Osrcar SR0     Oscar SR1
	//      100 MS/s		1			0				0
	//		50 M/s			1			1				0
	//		25 M/s			1			0				1
	//		10 M/s			1			1				1
	...
	pHwOptionText->u32RevMask = CSPLNDA_AOHW_OPT_FWREQUIRED_MASK;
...
int32	_CsDrvGetBoardType( uInt32 *u32MinBoardType, uInt32 *u32MaxBoardType )
{
	...
	*u32MinBoardType = CS16xyy_BT_FIRST_BOARD;
	*u32MaxBoardType = CS16xyy_LAST_BOARD;
	...
	case CS_FLASHINFO:
		{
			...
			if ( pCsFlashLayoutInfo->u32Addon )
			{
				...
				pCsFlashLayoutInfo->u32NumOfImages = SPLNDA_NUMBER_OF_FLASHIMAGES;
				pCsFlashLayoutInfo->u32ImageStructSize = sizeof(SPLNDA_FLASH_IMAGE_STRUCT);
				pCsFlashLayoutInfo->u32WritableStartOffset = 0;
				pCsFlashLayoutInfo->u32CalibInfoOffset = FIELD_OFFSET( SPLNDA_FLASH_LAYOUT, FlashImage ) + FIELD_OFFSET( SPLNDA_FLASH_IMAGE_STRUCT, Calib );
				pCsFlashLayoutInfo->u32FwInfoOffset = FIELD_OFFSET( SPLNDA_FLASH_LAYOUT, FlashImage ) + FIELD_OFFSET( SPLNDA_FLASH_IMAGE_STRUCT, FwInfo );
				pCsFlashLayoutInfo->u32FlashFooterOffset = FIELD_OFFSET( SPLNDA_FLASH_LAYOUT, Footer);
				pCsFlashLayoutInfo->u32FwBinarySize = 32*1024; // in bytes
				return CS_SUCCESS;
			}
			else if ( CS_DEVID_SPLENDA_PCIE == u32DeviceId || CS_DEVID_OSCAR_PCIE == u32DeviceId )
			{
				pCsFlashLayoutInfo->u32FlashType			= 1;
...
{
	if( u32BoardType >= CS16xyy_BT_FIRST_BOARD && u32BoardType <= CS16xyy_LAST_BOARD )
		return TRUE;		// Razors 
	else if( u32BoardType >= (CSNUCLEONBASE_BOARDTYPE | CS16xyy_BT_FIRST_BOARD) && u32BoardType <= (CSNUCLEONBASE_BOARDTYPE | CS16xyy_LAST_BOARD) )
		return TRUE;		// Razors PCI ex
	else if( u32BoardType >= CSE4abc_FIRST_BOARD && u32BoardType <= CSE4abc_LAST_BOARD )
		return TRUE;		// Oscars
	else
		return FALSE;
}
...
{
	CsSplendaDev	*pDev = (CsSplendaDev *) g_pBaseHwDll->GetAcqSystemCardPointer( DrvHdl );
