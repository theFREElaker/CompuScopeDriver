// Debug.h - Extended tracing/dumping/asserting utility set
//
...
#define  APP_TRACE_LEVEL		9
#define  IM_TRACE_LEVEL			8
#define  SOCKET_TRACE_LEVEL		7
#define  RM_TRACE_LEVEL			6
#define  DEMO_TRACE_LEVEL		5
#define  FS_TRACE_LEVEL		    5
#define  SSM_TRACE_LEVEL		4
#define  DISP_TRACE_LEVEL		3
#define  DRV_TRACE_LEVEL		2
#define  STATE_TRACE_LEVEL		1
#define  MISC_TRACE_LEVEL		0
...
public:
	...
	void  Boost ( DWORD dwNewPriority = THREAD_PRIORITY_TIME_CRITICAL )
	{
		...
		{
			m_dwPrevPriority = THREAD_PRIORITY_TIME_CRITICAL + 10;
		}
...
public:
	CToolHelp () : m_hSnapshot(INVALID_HANDLE_VALUE)
	{
...
inline void  CharToTchar ( LPCSTR psz, LPTSTR ptsz, int nLen )
{
	nLen = nLen;
...
inline void  TraceProcessorFeatures ( LPTSTR pszBuf, BOOL bIntel, DWORD dwFeatures, DWORD dwExtFeatures )
{
...
/*
inline void  GetProcessorInfo ( LPTSTR pszBuf )
{
...
inline void  ComputerCfgInfo ( LPTSTR pszBuf )
{
	...
	::GetSystemInfo(&si);
	int		nLen = 0;
	nLen += wsprintf ( pszBuf + nLen, 
		               _T("        OS type: %s\r\n"),
					  (vi.dwPlatformId == VER_PLATFORM_WIN32_NT ? _T("WIN32 NT") : _T("WINDOWS")));

	nLen += wsprintf ( pszBuf + nLen, 
		               _T("        OS version: major %u, minor %u, build %u\r\n"), 
					   vi.dwMajorVersion, vi.dwMinorVersion, vi.dwBuildNumber);

	nLen += wsprintf ( pszBuf + nLen,
		               _T("        Number of processors: %u\r\n"),
...
inline void  CrashReportCallback ( PEXCEPTION_POINTERS pei,
								   HANDLE hRptFile, 
						           LPCTSTR pcszRptPath,
								   LPCTSTR , 
								   ...
								   LPCTSTR pcszAppFileName )
{
	...
	wsprintf (pszMsg, _T("%s detected in the program \"%s\".\r\n\r\n")
			          ...
			          pszcErrorType, pcszAppFileName, pcszRptPath);		 
...
// dbgTracer helpers
//
inline bool  AddNameToPath ( LPTSTR pszPath, long lPathBufSize, LPCTSTR pcszFileName )
{
...
inline bool  AddNameToModulePath ( LPTSTR pszPath, long lPathBufSize, LPCTSTR pcszFileName )
{
	if ( NULL == pcszFileName  ||  0 == GetModuleFileName (GetModuleHandle(NULL), pszPath, lPathBufSize) )
		return  false;
	return  AddNameToPath (pszPath, lPathBufSize, pcszFileName);
}

inline LPCTSTR  GetFileNamePtr ( LPCTSTR pcszPath )
{
	if ( NULL == pcszPath  || 0 == pcszPath[0] )
		return  NULL;
...
inline LPTSTR  GetTstr ( const std::string& str )
{
...
inline std::string  ExtractPath ( PSTR pszPath, long nLen )
{
	int i;
	for ( i = nLen - 1; i >= 0; i-- )  
	{
...
inline std::string  ExtractName ( PSTR pszPath, long nLen )
{
	int i;
	for ( i = nLen - 1; i >= 0; i-- )  
	{
...
// _Tree class), the m_pActiveLevelsSet member is compilled to be a pointer to the 'vector'
...
// embarrassing to put up with them.
	...
	HANDLE		m_hMutex;  //!< Used to reveal whether symbolic info has already been initialized for the current process
	...
	__int64			m_i64PerfFreq, m_i64PerfCntAmendment, m_i64PerfCntAmendmentUTC;
						...
						DWORD& dwSec, DWORD& dwMilliSec, DWORD& dwMicroSec, bool bReal )
	{
		...
		if ( bReal )
		{
			t = ((__int64)(ftKernelMode.dwHighDateTime + ftUserMode.dwHighDateTime) * __int64(ULONG_MAX)
				+ (__int64)(ftKernelMode.dwLowDateTime + ftUserMode.dwLowDateTime)) / 10I64;
		}
		else  
		{
			FILETIME	ftCurTime;
			...
			t = (*(PINT64)&ftCurTime - *(PINT64)&ftCreationTime) / 10I64;
		}
		dwMicroSec = (DWORD)(t % 1000);
		dwMilliSec = (DWORD)((t / 1000) % 1000);
		dwSec = (DWORD)((t / 1000000) % 1000);
		return  t;
	}
	...
	void  DumpData ( LPTSTR pszMsg, int nSizeMsg, PVOID pvData, DWORD dwDataSize, DWORD dwParams, DWORD nAbsentElems )
	{
		...
		switch ( dwParams & DumpElemSizeMask )  
		{
			case  DumpBytes:
			{
				...
				for ( int i = 0; i < (int)nElems; i++ )
					dwLen += _stprintf_s (pszMsg + dwLen, nSizeMsg - dwLen, pszDumpFmt, pbData[i]);
			...
			case  DumpWords:
			{
				...
				for ( int i = 0; i < (int)nElems; i++ )
					dwLen += _stprintf_s (pszMsg + dwLen, nSizeMsg - dwLen, pszDumpFmt, pwData[i]);
			...
			case  DumpDwords:
			{
				...
				for ( int i = 0; i < (int)nElems; i++ )
					dwLen += _stprintf_s (pszMsg + dwLen, nSizeMsg - dwLen, pszDumpFmt, pdwData[i]);
			...
			case  DumpQwords:
			{
				...
				for ( int i = 0; i < (int)nElems; i++ )
					dwLen += _stprintf_s (pszMsg + dwLen, nSizeMsg - dwLen, pszDumpFmt, pqwData[i]);
		...
		if ( nAbsentElems )
		{
			for ( int i = 0; i < (int)(nAbsentElems * dwElemCellLen); i++, dwLen++ )
				pszMsg[dwLen] = _T(' ');
		}
	...
	{
		DWORD dwLen = 0, dwOpts = (dwForceOpts == -1 ? GetParams(nLevel) : dwForceOpts);

		pszFmt[0] = 0;

		if ( dwOpts & TraceLocation )
			dwLen += wsprintf (pszFmt + dwLen, _T("%s(%d) : "), m_pszFile, m_dwLine);
		...
		if ( dwOpts & TraceSystemTime )  
		{
			...
			if ( m_i64PerfFreq )  
			{
				...
				if ( dwOpts & TraceMicroseconds )
					dwMcs = (DWORD)((i64PerfCnt / 10) % 1000);
				dwMs = (DWORD)((i64PerfCnt / 10000) % 1000);
				dwSec = (DWORD)((i64PerfCnt / 10000000) % 60);
				dwMin = (DWORD)((i64PerfCnt / 600000000) % 60);
				dwHr = (DWORD)((i64PerfCnt / 36000000000) % 24);
			}
			else  
			{
				SYSTEMTIME	st;
			...
			if ( dwOpts & TraceMicroseconds )
				dwLen += wsprintf (pszFmt + dwLen, _T("%02d:%02d:%02d:%03d.%03d "), dwHr, dwMin, dwSec, dwMs, dwMcs);
			else
				dwLen += wsprintf (pszFmt + dwLen, _T("%02d:%02d:%02d.%03d "), dwHr, dwMin, dwSec, dwMs);
		}
		if ( dwOpts & TracePrThrID )  
		{
			if ( (dwOpts & TracePrThrID) == TracePrThrID )  
			{
				dwLen += wsprintf ( pszFmt + dwLen, (
					                dwOpts & TraceRtiHex ? _T("[%04X,%04X] ") : _T("[%05u,%05u] ")), 
									::GetCurrentProcessId(), 
									::GetCurrentThreadId() );
			}
			else  
			{
				dwLen += wsprintf ( pszFmt + dwLen,
					                (dwOpts & TraceRtiHex ? _T("[%04X] ") : _T("[%05u] ")),
		...
		if ( dwOpts & TracePrThrTime )
		{
			...
			if ( (dwOpts & TracePrThrTime) == TracePrThrTime )  
			{
				...
				if ( bMicroseconds )  
				{
					dwLen += wsprintf (pszFmt + dwLen, _T("{%04u:%03u.%03u,%04u:%03u.%03u} "), 
									  dwSecPr, dwMilliSecPr, dwMicroSecPr, 
									  dwSecPr, dwMilliSecPr, dwMicroSecPr);
				}
				else  
				{
					dwLen += wsprintf (pszFmt + dwLen, _T("{%04u.%03u,%04u.%03u} "), 
			...
			else  
			{
				...
				if ( bMicroseconds )
					dwLen += wsprintf (pszFmt + dwLen, _T("{%04u:%03u.%03u} "), dwSec, dwMilliSec, dwMicroSec);
				else
					dwLen += wsprintf (pszFmt + dwLen, _T("{%04u.%03u} "), dwSec, dwMilliSec);
			}
		}
		...
		if ( dwForceFnInfo & (TraceFnName | TraceFnArgTypes | TraceFnArgs) )
		{
			...
			if ( dwLen + strFn.length() + 3 >= MAX_TRACE_MSG_LEN )  {
			...
			dwLen += wsprintf (pszFmt + dwLen, _T("%s : "), pszFn);
			delete []  pszFn;
		}
		...
		{
			_ASSERTE (dwLen > 1);
			if ( pszFmt[dwLen - 2] != _T(':') )  
			{
				_ASSERTE (pszFmt[dwLen - 1] == _T(' '));
				lstrcpy (pszFmt + dwLen, _T(": "));
				dwLen += 2;
			}
		}
		wsprintf (pszFmt + dwLen, _T("%s"), pcszSrcFmt);
	}

	void  OutputMsgToMemBuffer ( LPCTSTR pszMsg, long nMsgSize )
	{
		size_t	tCurPos, tNewPos, tPrevPos, tBufferEnd;
		// Step 1: calculating nMsgSize - already done
start:
		// Step 2: 
		tCurPos = ::InterlockedExchangeAdd (m_pnBufferCurPos, nMsgSize);
		tNewPos = *m_pnBufferCurPos;
		tBufferEnd = *m_pnBufferEnd;	
		if ( (long)tCurPos > *m_pnBufferDeadLine )
		{
			...
			// Step 3:	
			tPrevPos = ::InterlockedCompareExchange (m_pnBufferCurPos, nMsgSize, long(tNewPos));
			if ( tPrevPos == tNewPos )  
			{
				// No context switches occured
				::InterlockedCompareExchange  (m_pnBufferEnd, long(tCurPos), long(tBufferEnd));
				tCurPos = 0;
			}
			else
		...
		{
			// Step 4:
			if ( (long)tNewPos > *m_pnBufferDeadLine )
			{
				// Update *m_pnBufferCurPos and *m_pnBufferEnd only if they 
				// has not been updated by a concurrent thread
				
				// Step 5:
				tPrevPos = ::InterlockedCompareExchange  (m_pnBufferCurPos, 0, long(tNewPos));
				if ( tPrevPos == tNewPos )
					// No context switches occured
					::InterlockedCompareExchange  (m_pnBufferEnd, long(tNewPos), long(tBufferEnd));
				else
					goto start;
			}
		}
		::CopyMemory (*m_ppszBuffer + tCurPos, pszMsg, nMsgSize);
	}
	...
	void  OutputMsgToFile ( int nLevel, LPCTSTR pszMsg, long nMsgSize, bool bSetRollbackInfo )
	{
		...
		GetParams(nLevel, &hFile, &pFilePosList);
	...
	static void OpenRptFile ( HANDLE *phFile, LPCTSTR pcszFile, DWORD dwFileOpts )
	{
		...
		*phFile = ::CreateFile ( pcszFile,
			                     GENERIC_WRITE,
								 FILE_SHARE_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE, NULL, OPEN_ALWAYS,
								 (dwFileOpts & TraceWriteThrough ? FILE_FLAG_WRITE_THROUGH : FILE_ATTRIBUTE_NORMAL),
								 NULL);
	...
	void InternalDump ( int nLevel, LPTSTR pszMsg, int nSizeMsg, PVOID pvData, DWORD dwDataSize, DWORD dwParams, LPCTSTR pcszFmt, va_list argptr )
	{
		if ( pcszFmt )
		{
			...
			_vstprintf_s (pszMsg, nSizeMsg, pszFmt, argptr);
			RemoveTrailingNewline (pszMsg);
			OutputMsg (nLevel, pszMsg);
		}
		pszMsg[0] = 0;
		DumpData (pszMsg, nSizeMsg, pvData, dwDataSize, dwParams, 0);
		OutputMsg (nLevel, pszMsg);
	}

	void  InternalDumpTable ( int nLevel, LPTSTR pszMsg, int nSizeMsg, PVOID pvData, DWORD dwDataSize, DWORD nCols, 
					  DWORD dwParams, LPCTSTR pcszFmt, va_list argptr )
	{
		...
		for ( int i = 0; i < (int)nRows; i++ )
		{
			...
			pszMsg[0] = 0;
			DumpData (pszMsg, nSizeMsg, pvData, dwCurRowDataSize, dwParams, (nRowSize - dwCurRowDataSize) / nElemSize);
	...
	void InternalTrace ( int nLevel, LPTSTR pszMsg, int nSizeMsg, DWORD dwFlags, LPCTSTR pcszFmt, va_list argptr )
	{
		if ( dwFlags & Prefix )
		{
		...
		{
			_vstprintf_s (pszMsg, nSizeMsg, pcszFmt, argptr);
		}
	...
	void  InternalTrace ( int nLevel, DWORD dwFlags, LPCTSTR pcszFmt, va_list argptr )
	{
		...
		InternalTrace (nLevel, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), dwFlags, pcszFmt, argptr);
	}
	...
	void  AddPathFromEnvVar ( std::string& strPath, LPSTR pszEnvVarName )
	{
		...
		if ( (n = ::GetEnvironmentVariable(pszEnvVarName, pszPath, MAX_PATH)) != 0 )
		{
#if !defined(_DEBUG)
...
#ifdef CS_WIN64
	static  BOOL  CALLBACK  EnumLoadedModulesProc ( PCSTR , DWORD64 dwModuleBase, 
													ULONG , PVOID pUserContext )
#else
	static  BOOL  CALLBACK  EnumLoadedModulesProc ( PCTSTR , ULONG dwModuleBase, 
													ULONG , PVOID pUserContext )
#endif
	{
		CHAR	pszPath[MAX_PATH];
		PATHS_SET	*pPathsSet = (PATHS_SET*)pUserContext;
		DWORD dw1 = ::GetModuleFileName(HMODULE(DWORD_PTR(dwModuleBase)), pszPath, MAX_PATH);
		if ( 0 == dw1 )
			return  TRUE;
		std::string strPath = ExtractPath (pszPath, dw1);
	...
	void  SetExtraNesting ()
	{
		...
		size_t tSymsrvStart = strSearchPath.find("symsrv*");
		if ( tSymsrvStart != NPOS )
		{
			tSymsrvStart += 7;
			size_t	tSymsrvEnd = strSearchPath.find("*", tSymsrvStart);
			if ( tSymsrvEnd != NPOS  &&  tSymsrvEnd != tSymsrvStart + 1 )
			{
				const std::string& strSymSrv = strSearchPath.substr(tSymsrvStart, tSymsrvEnd - tSymsrvStart);
				if ( strSymSrv.find(':') != NPOS )
					// Absolute path
					lstrcpyA (pszPath, strSymSrv.c_str());
				else
					// Relative path
					lstrcatA (pszPath, strSymSrv.c_str() +  (strSymSrv[0] == '\\' || strSymSrv[0] == '/' ? 1 : 0));
	...
	void  InitializeModule ()
	{
		...
		if ( th.IsAvailable()  &&  th.CreateSnapshot(CToolHelp::Module) )
		{
			...
			{ 
				...
				DWORD dw1 = ::GetModuleFileName(HMODULE(th.GetModuleBaseAddr()), pszPath, MAX_PATH);
				if ( dw1 )
				{
					std::string	strName = ExtractName(pszPath, dw1);
					::SymLoadModule ( GetCurrentProcess(),
						              NULL,
									  pszPath,
									  PSTR( strName.c_str() ),
									  DWORD(DWORD_PTR( th.GetModuleBaseAddr() )),
									  th.GetModuleSize());
				}
				break;
			} 
	...
	bool  GetSymSearchPath ( std::string& strSearchPath )
	{
		...
		DWORD   dw1 = ::GetModuleFileName(::GetModuleHandle(NULL), pszPath, MAX_PATH);
		...
		{
			strSearchPath = ExtractPath (pszPath, dw1);
		}
		...
		for ( PATHS_SET_IT	itPath = setPaths.begin(); itPath != setPaths.end(); itPath++ )  
		{
			strSearchPath += ";";
	...
	{
		long	_DLen = long(strNew.length() - strOld.length());
		size_t	_Pos = 0;
	...
	bool  GetTemplArgListPos ( std::string& strSymName, size_t tPrevPos, size_t& rtCurPos, 
							   size_t& rtTmplArgListStartPos, size_t& rtTmplArgListEndPos )
	{
		if ( strSymName[rtCurPos] != '<' )  
		{
			rtTmplArgListStartPos = rtTmplArgListEndPos = 0;
			return  false;
		}
		rtTmplArgListStartPos = rtCurPos - tPrevPos;
		int		nNesting = 1;
		while ( nNesting > 0  && (rtCurPos = strSymName.find_first_of("<>", ++rtCurPos)) != NPOS  )
		{
			if ( strSymName[rtCurPos] == '<' )
				nNesting++;
			else
		...
		_ASSERTE (nNesting == 0  &&  strSymName[rtCurPos] == '>');
		rtTmplArgListEndPos = rtCurPos - tPrevPos;
		return  true;
	}
	...
	int  GetPtrLevel ( const std::string& strType, size_t tTmplArgListEndPos, bool* pbRef = NULL )
	{
		int		nLevel = 0;
		size_t	tCurPos = tTmplArgListEndPos - 1;
		if ( pbRef )
			*pbRef = false;
		while ( (tCurPos = strType.find_first_of ("&*", ++tCurPos)) != NPOS )
		{
			if ( nLevel == 0 )  
			{
				if ( pbRef )
	...
	int  TraceStringPtr ( DWORD dwInfoFlags, char* pszBuf, DWORD* pdwArg, bool bWide )
	{
		...
		if ( bWide )  
		{
			...
			lstrcpynW (pwszString, LPWSTR(DWORD_PTR(*pdwArg)), MAX_STR_ARG_LEN);
			::WideCharToMultiByte (CP_ACP, 0, pwszString, -1, pszString, MAX_STR_ARG_LEN, NULL, NULL);
		}
		else
		{
			lstrcpynA (pszString, LPSTR(DWORD_PTR(*pdwArg)), MAX_STR_ARG_LEN);
		}
	...
	void  TraceStringObjArg ( const std::string& strArgType, DWORD dwInfoFlags, size_t tTmplArgListEndPos,
							  char* pszBuf, DWORD*& pdwArg, bool bWide, DWORD dwObjSize, DWORD dwOffset, int nPtrLevel = -1 )
	{
		nPtrLevel = nPtrLevel == -1 ? GetPtrLevel(strArgType, tTmplArgListEndPos) : nPtrLevel;
		switch ( nPtrLevel )  
		{
			case 0:
			...
			case 1:
				{
					...
					if ( *pdwArg )
						TraceStringPtr (dwInfoFlags, pszString, PDWORD(DWORD_PTR(*pdwArg)) + dwOffset, bWide);
					else
...
#define  IsType(StrType, Type)	(tTypeLen == sizeof(Type) - 1  &&  StrType.substr(0, tTypeLen) == Type)

	std::string  GetArgValue ( std::string& strArgType, DWORD dwInfoFlags, DWORD*& pdwArg, 
							   size_t tTmplArgListStartPos, size_t& tTmplArgListEndPos, bool& bUnknown )
	{
		char   pszBuf[64 + MAX_STR_ARG_LEN];
		size_t tTypeLen = strArgType.find_first_of(" ,&*<)");
		if ( tTypeLen == NPOS )
			tTypeLen = strArgType.length();
		bUnknown = false;
		if ( tTmplArgListStartPos )
		{
			// Template class
			...
			{
				// STL strings
				bool	bWide = strArgType.substr(tTmplArgListStartPos + 1, 4) == "WORD";
				if ( bWide )  
				{
					Replace (strArgType, "WORD", "wchar_t");
					tTmplArgListEndPos += 9;
				}
				else if ( strArgType.find("BYTE") != NPOS )  
				{
					Replace (strArgType, "BYTE", "u_char");
					tTmplArgListEndPos += 6;
				}
				
				try  
				{
					TraceStringObjArg (strArgType, dwInfoFlags, tTmplArgListEndPos, pszBuf, pdwArg, bWide, 4, 1);
				}
				catch ( ... ) 
				{
					wsprintfA (pszBuf, "0x%08X {??}", *pdwArg);
		...
		else  
		{
			...
			if ( IsType(strArgType, "CString") )
			{
				...
				try  
				{
					TraceStringObjArg (strArgType, dwInfoFlags, tTmplArgListEndPos, pszBuf,pdwArg, bWide, 1, 0, nPtrLevel);
				}
				catch ( ... ) 
				{
					wsprintfA (pszBuf, "0x%08X {??}", *pdwArg);
			...
			else if ( IsType(strArgType, "_bstr_t") )
			{
				try  
				{
					TraceStringObjArg (strArgType, dwInfoFlags, tTmplArgListEndPos, pszBuf,*(PDWORD*)pdwArg, true, DWORD(NPOS), 0, nPtrLevel);
				}
				catch ( ... ) 
				{
					wsprintfA (pszBuf, "0x%08X {??}", *pdwArg);
			...
			{
				TraceStringObjArg (strArgType, dwInfoFlags, tTmplArgListEndPos, pszBuf, pdwArg, false, 1, 0, 0);
				return  pszBuf;
			}
			else if ( nPtrLevel == 1  &&  
				      !(dwInfoFlags & TraceFnNoRawWideStrings)  && 
					  IsType(strArgType, "WORD")  &&  IsValidString(*(LPCWSTR*)pdwArg) )
			{
				Replace (strArgType, "WORD", "wchar_t");
				TraceStringObjArg (strArgType, dwInfoFlags, tTmplArgListEndPos, pszBuf, pdwArg, true, 1, 0, 0);
				return  pszBuf;
			}
			else if ( nPtrLevel == 0  ||  bRef  &&  nPtrLevel == 1 )
			{
				...
				if ( pcszFmt )
				{
					sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, (bRef ? **(PDWORD*)pdwArg++ : *pdwArg++));
					return  pszBuf;
				}
				...
				if ( pcszFmt ) 
				{
					if ( bRef )
					{
						sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, **(PDWORD64*)pdwArg++);
					}
					else 
					{
						sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, *(PDWORD64)pdwArg);
				...
				if ( pcszFmt )  
				{
					sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, d);
					return  pszBuf;
				}
				...
				if ( pcszFmt )
				{
					...
					sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, wVal, wVal);
					return  pszBuf;
				}
				...
				else
				{
					...
					sprintf_s (pszBuf, sizeof(pszBuf)/sizeof(pszBuf[0]), pcszFmt, *pdwArg, **(PDWORD*)pdwArg);
					pdwArg++;
				}
				return  pszBuf;
			}
...
#define  ProcessPrefix(str, prefix, flag) (str.find(prefix) == 0 ? str.erase(0, sizeof(prefix) - 1), flag = true : false )
	...
	bool  GetSymFromAddr ( DWORD dwAddr, DWORD dwStackAddr, DWORD dwInfoFlags, std::string& strSymbol )
	{
        ...
        PDWORD		pdwCurParam = PDWORD(DWORD_PTR(dwStackAddr + 2 * sizeof(DWORD) ));
		
		std::string	strSymName;
		size_t		tCurPos = 0, tPrevPos = 0;
		...
		DWORD	dwUndFlags = UNDNAME_NO_FUNCTION_RETURNS |
		...
		{
			size_t tCurPos = 0, tTmplArgListStartPos = 0, tTmplArgListEndPos = 0;
			boost.Boost();
			...
			strSymbol = pszBuf;
			if ( (dwInfoFlags & TraceFnStripTemplArgs  || !(dwInfoFlags & (TraceFnArgTypes | TraceFnArgs)))  &&
				  (tCurPos = strSymbol.find('<')) != NPOS  &&
				  GetTemplArgListPos (strSymbol, 0, tCurPos, tTmplArgListStartPos, tTmplArgListEndPos) )
			{
				strSymbol.erase (tTmplArgListStartPos + 1, tTmplArgListEndPos - tTmplArgListStartPos - 1);
			}
			goto  end;
		}
		...
		if ( tCurPos != NPOS )
		{
			...
			tPrevPos = strSymName.find(": ");
			if ( tPrevPos != NPOS )  
			{
				// This is a member function
				_ASSERTE (tPrevPos < tCurPos);
			...
			if ( bMemberFn  &&  (bStdCall  ||  bCDecl) )  
			{
				...
				strSymName.insert (tCurPos + 1, "this*,");
			}
			...
			tPrevPos = 0;
			size_t tTmplArgListStartPos = 0, tTmplArgListEndPos = 0;
			
			if ( ( 0 != (dwInfoFlags & TraceFnStripTemplArgs))  ||  
				 ( 0 == (dwInfoFlags & (TraceFnArgTypes | TraceFnArgs)) ) )
			{
				// Strip template arguments from template
				tPrevPos = strSymName.find('<');
				if ( tPrevPos != NPOS  &&  tPrevPos < tCurPos )
				{
					_ASSERTE (bMemberFn);
					GetTemplArgListPos (strSymName, 0, tPrevPos, tTmplArgListStartPos, tTmplArgListEndPos);
					strSymName.erase (tTmplArgListStartPos + 1, tTmplArgListEndPos - tTmplArgListStartPos - 1);
				}
				tPrevPos = 0;
			}
			Replace (strSymName, " &", "&");
				 ...
				 strSymName.find("(void)") == NPOS  &&  strSymName.find("()") == NPOS )
			{
				...
				strSymbol = strSymName.substr(tPrevPos, tCurPos - tPrevPos + 1);
				tPrevPos = ++tCurPos;
				while ( (tCurPos = strSymName.find_first_of("<,)", tCurPos)) != NPOS )
				{
					if ( GetTemplArgListPos (strSymName, tPrevPos, tCurPos, tTmplArgListStartPos, tTmplArgListEndPos) )
					{
						// Template args specification may be followed by a combination
						// of the 'const', '*' or '&' modifiers. Skip them.
						tCurPos = strSymName.find_first_of(",)", ++tCurPos);
						_ASSERTE (tCurPos != NPOS);
					}
					else if ( strSymName[tCurPos] == ')'  &&  
							  strSymName.find("...", tPrevPos) != NPOS )
					{
						break;
					}
					
					strArgType = strSymName.substr(tPrevPos, tCurPos - tPrevPos);
					if ( dwInfoFlags & TraceFnArgs )  
					{
						if ( !bUnknown  &&  --nArgsToSkip < 0 )  
						{
							...
							strArgValue += GetArgValue (strArgType, dwInfoFlags, pdwCurParam, 
														tTmplArgListStartPos, tTmplArgListEndPos, bUnknown);
						}
						else
							strArgValue = "=??";
					}
					if ( (0 != (dwInfoFlags & TraceFnStripTemplArgs)) && (0 != tTmplArgListStartPos) )
					{
						strArgType.erase (tTmplArgListStartPos + 1, 
										  tTmplArgListEndPos - tTmplArgListStartPos - 1);
					}
					strSymbol += strArgType;
					strSymbol += strArgValue;
					strSymbol += strSymName[tCurPos];
					tPrevPos = ++tCurPos;
				}
				strSymbol += strSymName.substr(tPrevPos);
			}
			else  
			{
				if ( dwInfoFlags & (TraceFnArgTypes | TraceFnArgs) )
					...
					strSymbol = strSymName.substr(tPrevPos, tCurPos - tPrevPos);
			}
		}
		else 
		{
			...
			if ( strSymName[0] == '_' )  
			{
				if ( (tCurPos = strSymName.find("@")) != NPOS  &&
					 _stscanf_s (strSymName.c_str() + tCurPos + 1, "%u", &nBytes) == 1 )
				{
			...
			else 
			{
				...
				if ( ::SymGetModuleInfo(GetCurrentProcess(), dwAddr, &ihm)  &&  ihm.ModuleName[0] )
				{
					...
					if ( tCurPos )
					{
						...
						strFn += '!';
						strFn += strSymbol.substr(0, tCurPos);
	...
	void  ProtectedGetRawDwordList ( LPSTR pszArgs, int& nLen, DWORD*& ptCurPos, DWORD nDwords, PSTR , PSTR )
	{
		__try  
		{
			for ( int i = 0; i < (int)nDwords; i++ )
			{
				nLen += wsprintfA (pszArgs + nLen, "0x%08X,", *ptCurPos++);
			}
		}
	...
	{
		PDWORD	pFrame = PDWORD(DWORD_PTR(psf->AddrFrame.Offset)),
				pPrevFrame;

		psf->AddrPC.Offset = pFrame[1];
		pPrevFrame = pFrame;
		pFrame = PDWORD(DWORD_PTR(pFrame[0])); // Proceed to next frame on the stack
		if ( DWORD(DWORD_PTR(pFrame)) & 3 )    // Frame pointer must be aligned on a DWORD boundary
			return  FALSE;                  
		...
		if ( ::IsBadWritePtr(pFrame, sizeof(PVOID)*2) )
			return  FALSE;
		psf->AddrFrame.Offset = DWORD(DWORD_PTR(pFrame));
		return  TRUE;
	}
					   ...
					   bool bImagehlpStackWalk = true )
	{
		...
		for (;;)
		{
			...
			if ( nCurLevel++ < (int)nExtraNesting )
				continue;
			...
			if ( *m_pdwStackTraceOpts & TraceLocation )
			{
				...
				strLocation += " : ";
				nMaxLocationLen = max(nMaxLocationLen, (int)strLocation.length());
			}
...
#else
		nLevel;pCtx;nExtraNesting; hThread; bImagehlpStackWalk;
#endif
	}
	...
	std::string  GetCallerFn ( DWORD dwInfoFlags, long nExtraNesting = 0 )
	{
		...
		return  strFn;
#else
		std::string		strFn(_T("GetCallerFn is not supported in x64") );
		dwInfoFlags; nExtraNesting;
		return  strFn;
#endif
	}
...
#define CheckException( x )  case  EXCEPTION_##x:  return _T(#x);
	...
	void  WriteCrashReportTitle ( LPTSTR pszBuf )
	{
		...
		if ( *m_pdwCrashRptOpts & TraceAddInfoMask )
		{
			...
			try 
			{  
				PrepareFmtString (CRASH_REPORT_LEVEL, pszFmt, pszBuf, 0);  
			}
			catch (...)
			{
				pszFmt = pszBuf;
...
#define	 BUFFERED_TRACES	_T(" -> Buffered traces:\r\n")
	...
	void  FlushTraceBuffer ( int nLevel )
	{
		...
		*m_pnBufferEnd = *m_pnBufferDeadLine;
		...
		{
			while ( pszBuffer[nCurPos++] )
			{
			}	// skips partially overwritten message
	...
	void  Dump ( PVOID pvData, DWORD dwDataSize, DWORD dwParams = 0, LPCTSTR pcszFmt = NULL, ... )
	{
		...
		InternalDump (0, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), pvData, dwDataSize, dwParams, pcszFmt, argptr);
		va_end (argptr);
	}
	...
	void  Dump ( int nLevel, PVOID pvData, DWORD dwDataSize, DWORD dwParams = 0, LPCTSTR pcszFmt = NULL, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalDump (nLevel, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), pvData, dwDataSize, dwParams, pcszFmt, argptr);
		va_end (argptr);
	}
					  ...
					  DWORD dwParams = 0, LPCTSTR pcszFmt = NULL, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalDumpTable (0, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), pvData, dwDataSize, nCols, dwParams, pcszFmt, argptr);
		va_end (argptr);
	}
					  ...
					  DWORD dwParams = 0, LPCTSTR pcszFmt = NULL, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalDumpTable (nLevel, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), pvData, dwDataSize, nCols, dwParams, pcszFmt, argptr);
		va_end (argptr);
	}
	...
	void  TrAssert ( BOOL bExpr, LPCTSTR pcszFmt, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalTrace (0, pszMsg, sizeof(pszMsg)/sizeof(TCHAR), m_dwTraceFlags, pcszFmt, argptr);
		va_end (argptr);
		...
		bool bTest = sizeof(TCHAR) == sizeof(WCHAR);
		if( bTest )
		{
	...
	void  TraceExpected ( LPCTSTR pcszFmt, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalTrace (0, GetExpectedBuf(), MAX_TRACE_MSG_LEN, Lf, pcszFmt, argptr);
		va_end (argptr);
	}
	...
	void  TraceResulted ( LPCTSTR pcszFmt, ... )
	{
		...
		va_list		argptr; 
		va_start (argptr, pcszFmt);
		InternalTrace (0, GetResultedBuf(), MAX_TRACE_MSG_LEN, Lf, pcszFmt, argptr);
		va_end (argptr);
...
inline LONG  WINAPI  __dbg_util::dbgTracer::DbgUtilUnhandledExceptionFilter ( PEXCEPTION_POINTERS pExceptionInfo )
{
...
public:
	...
	{
	#if defined (_DEBUG) || defined  (RELEASE_TRACE)
		int anTraceLevel[TRACE_LEVELS], nLevels(0);
		int anFileOptions[TRACE_LEVELS];
		TCHAR aFileName[TRACE_LEVELS][MAX_PATH];
		
		TCHAR Subkey[50] = _T("SOFTWARE\\Gage\\Trace\\Level0");
		HKEY	hKey;
		TCHAR	t;

		for(t = 0; t < TRACE_LEVELS; t++)
		{
			Subkey[25] = _T('0') + t;
			aFileName[t][0] = 0;
			anFileOptions[t] = -1; //disable by default
			DWORD dwFlags = KEY_QUERY_VALUE;
			if (Is32on64())
				dwFlags |= KEY_WOW64_64KEY;
			if (ERROR_SUCCESS == ::RegOpenKeyEx(HKEY_CURRENT_USER, Subkey, 0, dwFlags, &hKey))
			{
				DWORD dwSize(sizeof(anFileOptions[t]));
				if( ERROR_SUCCESS != ::RegQueryValueEx(hKey,_T("Stream"), NULL, NULL, LPBYTE(&anFileOptions[t]), &dwSize) ) 
					anFileOptions[t] = -1; //disable by default
			}
			if( 0 <= anFileOptions[t] )
			{
				DWORD dwSize(sizeof(aFileName[t]));
				if( ERROR_SUCCESS != ::RegQueryValueEx(hKey,_T("FileName"), NULL, NULL, LPBYTE(aFileName[t]), &dwSize))
					{	anFileOptions[t] = 0;aFileName[t][0] = 0;	}
				anTraceLevel[nLevels++] = t;
			}
			::RegCloseKey(hKey);
		}
		if(0 != nLevels)
		{
			SET_ACTIVE_TRACE_LEVELS(nLevels, anTraceLevel);
			for(t = 0; t < TRACE_LEVELS; t++)
			{
				if(0 != aFileName[t][0])
				{
					SET_TRACE_FILE(t, aFileName[t], anFileOptions[t] );
					SET_TRACE_OPTS (t, TraceSystemTime | TraceTID | TraceMicroseconds);
				}
			}
		}
		else
		{
			SET_MAX_TRACE_LEVEL( 0 );
		}
