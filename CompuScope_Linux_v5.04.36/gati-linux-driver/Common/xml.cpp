...
XMLElement* XMLHelper :: ParseElementTree(XMLHeader* hdr,XMLElement* parent,char* tree,char** ,int& iParseStatus)
   {
   char *a1,*a2,*a3,*a4,*a5;
   char c1,c2;
...
int XML :: Save(const char* file,int SaveMode,int TargetMode,class XMLTransform* eclass,class XMLTransformData* edata)
   ...
   if (!fp)
	...
	if (TargetMode == 3)
		...
		// Hdr utf-16
		if (hdr)
			hdr->SetEncoding((char *)"UTF-16");
		}
	if (TargetMode == 0)
		{
		if (hdr)
			hdr->SetEncoding((char *)"UTF-8");
		}
...
int XML :: PartialLoad(const char*,const char* map)
	{
	if (!map)
		return 0;
	return 0;//
...
void XMLHeader :: Export(FILE* fp,int SaveMode,int TargetMode,class XMLTransform*,class XMLTransformData*)
   {
   if (TargetMode == 1)
      {
      if (SaveMode == 0)
...
// Global functions
Z<char>* XML :: ReadToZ(const char* file,XMLTransform* eclass,class XMLTransformData*)
   {
   int status = 0;
   ...
   if (eclass == 0)
      {
      y = new Z<char>(S + 32);
      status = fread((*y).operator char *(),1,S,fp);
      fclose(fp);
      }
   else
      {
      Z<char> yy(S + 32);
      y = new Z<char>(S + 32);
      status = fread(yy.operator char *(),1,S,fp);
      fclose(fp);
